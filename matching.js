const getSecondsBetweenTimestamps = (timestamp1, timestamp2) =>
 ...
 export function getMatchingConfidence(pulse_db, pulse_client){
   ...
   else  if(secondsInBetween <= 900)  return Number(0.70 + deviceMatchingScore(pulse_db, pulse_client, 0.075)).toFixed(2); //max: 85%
   else  if(secondsInBetween <= 1200) return Number(0.50 + deviceMatchingScore(pulse_db, pulse_client, 0.1)).toFixed(2);   //max: 70%
   return 0;
 }